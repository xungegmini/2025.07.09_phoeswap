...
import { PlusCircleIcon, MinusCircleIcon, ChevronDownIcon as ChevronDownMiniIcon } from '@heroicons/react/24/outline'; 
import { toast } from 'sonner';

// ZMIANA: Używamy krótszej, zweryfikowanej listy tokenów, aby uniknąć zawieszania.
        ...
        <div className="flex-1 w-full py-8 px-4">
            <div className="max-w-2xl mx-auto">
                ...
                <div className="bg-phoenix-container-bg p-6 rounded-xl shadow-xl border border-phoenix-border mb-10">
                    ...
                    {/* ZMIANA: Przyciski wyboru tokena są nieaktywne, jeśli portfel nie jest połączony */}
                    <div className="mb-4">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => openModal('A')} disabled={!connected} className="flex-shrink-0 flex items-center bg-phoenix-bg border border-phoenix-border hover:border-phoenix-accent text-phoenix-text-primary py-2.5 px-4 rounded-lg text-sm font-medium min-w-[150px] justify-between disabled:opacity-50 disabled:cursor-not-allowed">
                                {tokenA ? (<>{tokenA.logoURI && <img src={tokenA.logoURI} alt={tokenA.symbol} className="h-5 w-5 mr-2 rounded-full" />}{tokenA.symbol}</>) : 'Select'}<ChevronDownMiniIcon className="h-5 w-5 ml-1 text-phoenix-text-secondary" />
                            </button>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => openModal('B')} disabled={!connected} className="flex-shrink-0 flex items-center bg-phoenix-bg border border-phoenix-border hover:border-phoenix-accent text-phoenix-text-primary py-2.5 px-4 rounded-lg text-sm font-medium min-w-[150px] justify-between disabled:opacity-50 disabled:cursor-not-allowed">
                                {tokenB ? (<>{tokenB.logoURI && <img src={tokenB.logoURI} alt={tokenB.symbol} className="h-5 w-5 mr-2 rounded-full" />}{tokenB.symbol}</>) : 'Select'}<ChevronDownMiniIcon className="h-5 w-5 ml-1 text-phoenix-text-secondary" />
                            </button>
                    ...
                    {/* ZMIANA: Zamiast MyMultiButton, używamy prostego przycisku, który pasuje stylistycznie */}
                    {connected ? (
